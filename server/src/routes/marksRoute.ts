...
studentMarksRouter.post('/:studentId/marks/:subjectName', async (req: Request, res: Response) => {
...
studentMarksRouter.get('/:studentId/total-marks', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Use non-null assertion operator (!) to inform TypeScript that student.marks will not be undefined
    const totalMarks = Object.values(student.marks!).reduce((total, mark) => total + mark, 0);
