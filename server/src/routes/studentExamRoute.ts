...
studentExamEntryRouter.post("/student-exam-entry", async (req: Request, res: Response) => {
    try {
...
studentExamEntryRouter.get("/student-exam-entry", async (req: Request, res: Response) => {
    try {
        const studentExamEntries = await StudentExamEntry.find().populate("student exam");
...
studentExamEntryRouter.delete("/student-exam-entry", async (req: Request, res: Response) => {
    try {
        // Delete all student exam entries
        await StudentExamEntry.deleteMany({});

        res.status(200).json({ message: "All student exam entries deleted successfully." });
    } catch (error) {
