...
const AddStudentMarksTotal: React.FC = () => {
  ...
  useEffect(() => {
    const fetchStudentData = async () => {
      try {
        ...
        const marksResponse = await fetch('http://localhost:5100/getAllMarks');
        if (!marksResponse.ok) {
        ...
        const namesResponse = await fetch('http://localhost:5100/getAllStudentNames');
        if (!namesResponse.ok) {
  ...
  const calculateTotalMarks = (marks: { subjectName: string, marks: number }[]) => {
  ...
  const subjects = [...new Set(studentMarksList.flatMap(student => student.subjectMarks.map(subject => subject.subjectName)))];
        ...
        <thead>
          <tr>
            ...
            {subjects.map((subject, index) => (
              <th key={index} className="px-4 py-2">{subject}</th>
            ))}
        ...
        <tbody>
          {sortedStudentMarksList.map((studentMark, studentIndex) => (
            <tr key={studentIndex}>
              ...
              <td className="border px-4 py-2">{studentNames[studentMark.studentId]}</td>
              {subjects.map((subject, subjectIndex) => {
                const marks = studentMark.subjectMarks.find(item => item.subjectName === subject);
                return <td key={subjectIndex} className="border px-4 py-2">{marks ? marks.marks : 0}</td>;
              })}
              <td className="border px-4 py-2">{calculateTotalMarks(studentMark.subjectMarks)}</td>
            </tr>
          ))}
        </tbody>
      </table>
